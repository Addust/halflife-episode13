...
/obj/item/radio/proc/make_syndie() // Turns normal radios into Syndicate radios!
	qdel(keyslot)
	keyslot = new /obj/item/encryptionkey/syndicate/uprising()
...
/obj/item/radio/proc/talk_into_impl(atom/movable/talking_movable, message, channel, list/spans, datum/language/language, list/message_mods)
	...
	*/
	...
	if (special_channels & RADIO_SPECIAL_CENTCOM && (freq == FREQ_CENTCOM || freq == FREQ_CTF_RED || freq == FREQ_CTF_BLUE || freq == FREQ_CTF_GREEN || freq == FREQ_CTF_YELLOW || freq == FREQ_STATUS_DISPLAYS))
	...
	if(isliving(talking_movable))
		...
		if(radio_noise && talking_living.can_hear() && volume_modifier && signal.frequency != FREQ_COMMON)
			var/sound/radio_noise = sound(sound('hl13/sound/items/radio_chatter.ogg', volume = volume_modifier)) //hl13 edit
